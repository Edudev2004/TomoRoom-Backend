    ...
    ffmpeg(finalUrl)
      ...
      .on("progress", (progress) => {
        if (progress.percent && progress.percent > 0) {
          ...
          let extra = "";
          if (progress.timemark) extra += ` [${progress.timemark}]`;
          if (progress.targetSize) extra += ` ${progress.targetSize}KB`;
          record.status = extra ? `descargando...${extra}` : "downloading";
        }
        record.updatedAt = Date.now();
      })
      .on("error", async (err) => {
...
async function downloadFromUrl(record, candidate) {
  ...
  if (isHls) {
    ...
    response.data.on("data", (chunk) => {
      ...
      record.progress = Math.min(90, record.progress + 1);
      const mb = (record.downloadedBytes / 1024 / 1024).toFixed(2);
      record.status = `descargando... (${mb} MB)`;
    });
